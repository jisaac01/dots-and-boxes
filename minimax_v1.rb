...
class PerfectPlayer
  ...
  def get_best_move
    ...
    if the_rest.empty? && three_sided_moves.size < 3
      ...
      debug board.to_s
      next_move = minimax(true, board.player_id, 0)[:move]
    else
  ...
  def minimax(maximize, current_player, depth)
    ...
    if (Time.now > $start_time + TIMEOUT) || depth >= @max_depth || BoardLogic.end_state?(board)
      score = BoardLogic.score(board, !maximize) #because we're scoring for the previous turn
      debug "End State: depth #{depth}, score: #{score}, maximize: #{maximize}", depth
      ...
      available_moves.each_with_index do |move, index|
        debug "current_move: #{move}", depth
        debug "best_score: #{best_score}, best_move: #{best_move}"
        ...
        if maximize
          result = minimax(next_maximize, next_player, depth + 1)
