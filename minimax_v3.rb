...
class PerfectPlayer
  ...
  def get_ordered_moves
    three_sided_moves, the_rest = BoardLogic.all_possible_moves(board).partition do |move| 
      neighbor = move.neighbor(board)
      Square.three_sided?(board.squares[move.row][move.col]) || (neighbor && Square.three_sided?(board.squares[neighbor.row][neighbor.col]))
    end
  
    two_sided_moves, the_rest = the_rest.partition do |move|
      neighbor = move.neighbor(board)
      Square.two_sided?(board.squares[move.row][move.col]) || (neighbor && Square.two_sided?(board.squares[neighbor.row][neighbor.col]))
    end
      
    [three_sided_moves, the_rest, two_sided_moves]
  end
  ...
  def get_best_move
    ...
    if the_rest.empty? && three_sided_moves.size < 3 && (three_sided_moves.size < two_sided_moves.size)
      @max_depth = 3
...
class BoardLogic
  ...
  def self.all_possible_moves(board)
    ...
    board.each_square do |row, column|
      if !Square.player?(board.squares[row][column])
        Square.open_sides(board.squares[row][column]).each do |side|
          move = Move.new(row, column, side)
          moves << move unless moves.include?(move.neighbor(board))
        end
      end
    end
    moves
